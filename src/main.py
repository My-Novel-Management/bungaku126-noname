...
def ep_catch_her(w: World):
    return w.episode("彼女を拾う",
            ...
            w.plot_note("$mumeは機嫌のいい$ishiから映画のチケットをもらう"),
            w.plot_note("$ishiはそのうち有名になると$mumeに宣言する"),
            w.plot_turnpoint("$ishiに捨てられる", about="yuna"),
            w.plot_note("$ishiは$yunaを捨てて、家を出ていった"),
            w.plot_note("$mumeは大声で二人が口論して別れ話をしているのを聞いていた"),
            w.plot_turnpoint("$mumeは捨てられた$yunaを拾う"),
            w.plot_note("二人が別れた翌日、家の前に$yunaの姿があった"),
            w.plot_note("彼ともう一度話し合いたいという彼女を家に入れた"),
            w.plot_develop("$mumeは$yunaから事情を聞く"),
            w.plot_develop("$ishiが戻ってくるまでこの部屋を使えばいいと提案した"),
            )

def ep_streamer(w: World):
    return w.episode("ゲーム実況者",
            w.plot_develop("$yunaは何もせずに部屋でじっとしていた"),
            w.plot_develop("見かねた$mumeは少し声を掛ける"),
            w.plot_develop("彼女は特に趣味もなく、普段は$ishiがゲーム実況をしていたのを見ていたと言う"),
